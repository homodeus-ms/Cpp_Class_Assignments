...
template<typename T>
void PrintMinMax(SmartStack<T> ss)
{
	cout << "Max: " << ss.GetMax() << " / " << "Min: " << ss.GetMin() << endl;
}

template<typename T>
void PrintValues(SmartStack<T> ss)
{
	cout << "Average: " << ss.GetAverage() << " / "
		<< "V: " << ss.GetVariance() << " / "
		<< "S: " << ss.GetStandardDeviation() << endl;
}

template <typename T>
void PrintQueueStack(queue<stack<T>>& qs)
{
	queue<stack<T>> queueKeep;
	stack<T> stackKeep;

	while (qs.size() != 0)
	{
		stack<T> poped = qs.front();
		queueKeep.push(poped);
		qs.pop();
		size_t count = poped.size();

		while (count != 0)
		{
			stackKeep.push(poped.top());
			poped.pop();
			--count;
		}

		count = stackKeep.size();
...
int main()
{
    ...
    cout << ss.GetAverage() << " " << ss.GetVariance() << " " << ss.GetStandardDeviation() << endl;
    ...
    cout << qs.mTotalElementCount << " / " << qs.mStackCount << endl;

    cout << qs.GetMin() << endl;

#endif

#if 0   // SmartQueue 대입연산자 체크
	SmartQueue<float> q1;
	q1.Enqueue(3.14f);
	q1.Enqueue(1.2f);
	q1.Enqueue(9.99f);

	cout << q1.GetCount() << " / " << q1.GetAverage() << endl;

	SmartQueue<float> q2;
	q2.Enqueue(-1.01f);

	cout << q2.GetCount() << " / " << q2.GetAverage() << endl;
